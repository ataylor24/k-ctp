    ...
    for size in algos.keys():
            for k in algos[size]:
                for alg in algos[size][k]:
                        ...
                        #window = 5
                        for window in [5,1]:
                            for num_sources in range(size-3,15,-window): #range(0,15,window):#
                                for i in range(num_sources, num_sources - window, -1): #range(num_sources, num_sources + window, 1):#  
                                    try:
                                        ...
                                        print(scaler.fit_transform(dfs[alg][~np.isnan(dfs[alg])].to_numpy())[i])
                                        sns.distplot(scaler.fit_transform(dfs[alg][~np.isnan(dfs[alg])].to_numpy())[i], kde= True)
                                ...
                                plt.title('Density of hitting times from source(s) %s%s to target=31' % (num_sources - window + 1, "..." + str(num_sources) if window != 1 else ""))
