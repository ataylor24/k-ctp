                            ...
                            for num_sources in range(0,15,window): #range(size-3,15,-window): #
                                for i in range(num_sources, num_sources + window, 1):#range(num_sources, num_sources - window, -1): #  
                                    try:
                                        ...
                                        sns.kdeplot(np.log10(dfs[alg][~np.isnan(dfs[alg])][i]), shade=True)
                                ...
                                plt.title('Log-Scaled Density of hitting times from source(s) %s%s to target=31' % (num_sources - window + 1, "..." + str(num_sources) if window != 1 else ""))
