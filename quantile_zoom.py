    ...
    for size in algos.keys():
            for k in algos[size]:
                for alg in algos[size][k]:
                        ...
                        #window = 5
                        upper_quartile_limits = [0.75, 0.5]
                        lower_quartile_limits = [0, 0.25]
                        for window in [5,1]:
                            for num_sources in range(0,15,window): #range(size-3,15,-window): #
                                for upper_quartile_limit in upper_quartile_limits:
                                    for lower_quartile_limit in lower_quartile_limits:
                                        for i in range(num_sources, num_sources + window, 1):#range(num_sources, num_sources - window, -1): #
                                            try:
                                                ...
                                                #sns.kdeplot(dfs[alg][~np.isnan(dfs[alg])][i], shade=True)
                                                hitting_time_df = dfs[alg][~np.isnan(dfs[alg])][i]
                                                lower_quartile = hitting_time_df.quantile(lower_quartile_limit)
                                                upper_quartile = hitting_time_df.quantile(upper_quartile_limit)
                                            
                                                zoomed_ht_df = hitting_time_df.loc[hitting_time_df < upper_quartile].loc[hitting_time_df > lower_quartile]
                                                print(zoomed_ht_df)
                                                sns.kdeplot(zoomed_ht_df, shade=True)
                                        ...
                                        plt.ylabel('Density')
                                        plt.xlabel('Hitting Time (# of steps taken)')
                                        ...
                                        plt.title('Density of hitting times from source(s) %s%s to target=31, %s< Quartile < %s' % (num_sources - window + 1, "..." + str(num_sources) if window != 1 else "", lower_quartile_limit, upper_quartile_limit))
                                        plt.savefig("%s_hist_by_source(source(s)=%s%s)_zoomed_%s<Q<%s.png" % (alg, num_sources, " to " + str(num_sources + window - 1) if window != 1 else "", lower_quartile_limit, upper_quartile_limit))
