...
def graph(graph_type):
    ...
    for size in algos.keys():
        for k in algos[size]:
            for alg in algos[size][k]:
                plt.figure()
                buckets = int(float(runs_per_graph_size[size])/10) if runs_per_graph_size[size] > 30 else runs_per_graph_size[size]
                fil_name = os.path.join(os.getcwd(), ("hist_%s_%s_%s_%s.png" % (graph_type, size, k,alg)))
